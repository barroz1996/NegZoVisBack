...
class FilesManager(ABC):
    ...
    def __init__(self, input_path, output_path, name="", hdf=False, write_prob_data=True):
        self.dataset = None
        ...
        self.write_as_hdf = hdf
        self.write_prop_data = write_prob_data
    ...
    def read_dataset(self):
        ...
        if not (os.path.exists(entity_class_relations_path)) or self.dataset is None:
    ...
    def write_symbolic_time_series(self, sts):
        if self.write_as_hdf:
            sts.to_hdf(
                self.__get_symbolic_time_series_path(),
                index=False,
                key=FileNames.SYMBOLIC_TIME_SERIES.replace("-", "_"),
            )
        else:
    ...
    def write_KL(self, symbolic_time_intervals):
        ...
        for class_id, class_df in symbolic_time_intervals_with_classes.groupby(
            by=EntityClassRelationsColumns.ClassID
        ):
    ...
    def write_folds(self, folds):
        ...
        for idx, (train, test) in enumerate(folds):
            ...
            train_fm = FilesManager(
                train_path,
                train_folder_path,
                f"{fold_name}-train",
                hdf=self.write_as_hdf,
                write_prob_data=self.write_prop_data,
            )
            train_fm.read_dataset()
            train_fm.write_entity_class_relations(entity_class_relations)

            test_fm = FilesManager(
                test_path,
                test_folder_path,
                f"{fold_name}-test",
                hdf=self.write_as_hdf,
    ...
    def __extract_entity_class_relations(self):
        ...
        entity_class_relations = dataset[
            dataset[DatasetColumns.TemporalPropertyID] == FilesManager.CLASS_IDENTIFIER
        ].copy()

        dataset.drop(entity_class_relations.index, axis="index", inplace=True)
        if self.write_prop_data:
        ...
        entity_class_relations = entity_class_relations.drop(
            [DatasetColumns.TemporalPropertyID, DatasetColumns.TimeStamp], axis="columns"
        )
        entity_class_relations = entity_class_relations.rename(
            columns={DatasetColumns.TemporalPropertyValue: EntityClassRelationsColumns.ClassID}
        )
    ...
    def __get_symbolic_time_series_path(self):
        suffix = "h5" if self.write_as_hdf else "csv"
        file_name = f"{self.name}{FileNames.SYMBOLIC_TIME_SERIES}.{suffix}"
        return os.path.join(self.output_path, file_name)

    @staticmethod
    def parse_KL_to_time_intervals(kl_str):
        file_lines = kl_str.split("\n")[2:]
        entities_to_intervals = zip(
            list(map(lambda entity_line: entity_line.split(";")[0], file_lines[::2])),
            list(
                map(
                    lambda entity_intervals: list(
        ...
        intervals = reduce(
            lambda x, y: x + y,
            map(
                lambda entity_to_intervals: list(
                    map(
                        lambda interval: pd.Series(
                            {
                        ...
                        entity_to_intervals[1],
                    )
                ),
                entities_to_intervals,
            ),
        )
    ...
    def parse_time_intervals_to_KL(time_intervals_df):
        ...
        intervals_str = time_intervals_df.groupby(by=DatasetColumns.EntityID).apply(
            lambda entity_df: ";".join(
                map(
